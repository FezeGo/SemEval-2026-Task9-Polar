...
# Paths
# =========================
TRAIN_DIR = "data/test_phase/subtask1/train"
DEV_DIR = "data/test_phase/subtask1/dev"
OUTPUT_DIR = "outputs/subtask1"
MODEL_DIR = "checkpoints/subtask1"
...
def compute_pos_weight(df: pd.DataFrame) -> float:
    pos_count = int((df["polarization"] == 1).sum())
    neg_count = int((df["polarization"] == 0).sum())
    pos_weight = neg_count / max(pos_count, 1)

    print(f"pos_count={pos_count}, neg_count={neg_count}, pos_weight={pos_weight:.4f}")
...
def extract_best_epoch(trainer) -> int:
    """
    Try to recover the best epoch from trainer.state.log_history.
    Falls back to the final epoch if needed.
    """
    state = trainer.state
